$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.status == "success") {
                    ...
                    localStorage.removeItem('addData');
                    toastr.success('Pharmacy Purchase Added To Main TableSuccessfully', 'Added!');
                }
            },
            error: function (err) {
    ...
    ///////////// ------------------ Edit Pharmacy Purchase ajax part start ---------------- /////////////////////////////
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.status == "success") {
                    ...
                    $('#status').val('1');
                    localStorage.removeItem('editData');
                    toastr.success('Pharmacy Purchase Main Updated Successfully', 'Updated!');
                }
            },
            error: function (err) {
                let error = err.responseJSON;
    ...
    /////////////// ------------------ Delete Pharmacy Purchase Main Ajax Part Start ---------------- /////////////////////////////
    ...
    /////////////// ------------------ Delete Pharmacy Purchase Main Ajax Part End ---------------- /////////////////////////////
    ...
    PaginationAjax({ type: 6, method: 'Purchase', status: $('#status').val() });

    SearchAjax({ type: 6, method: 'Purchase', status: $('#status').val() });
    
    SearchByDateAjax({ type: 6, method: 'Purchase', status: $('#status').val() });

    SearchPaginationAjax({ type: 6, method: 'Purchase', status: $('#status').val() });
    ...
    $(document).on('change', '#status', function(e){
        ...
        LoadData(urls.search, {search, status, startDate, endDate, method, type, searchOption}, '.inv-purchase');
    });
            ...
            success: function (res) {
                if(res.status === 'success'){
                    ...
                    inventories.forEach(pharmacy => {
                        let productGrid = {
                            product: pharmacy.tran_head_id,
                            name: pharmacy.head.tran_head_name,
                            groupe: pharmacy.tran_groupe_id,
                            quantity: pharmacy.total_quantity,
                            unit: pharmacy.unit_id,
                            cp: pharmacy.cp,
                            mrp: pharmacy.mrp,
                            totAmount: pharmacy.total_amount,
                            expiry: pharmacy.expiry_date
                        };
