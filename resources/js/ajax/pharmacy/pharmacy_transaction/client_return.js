$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                batches.forEach(batch => {
                    rows += `<tr data-id="${batch.tran_head_id}" data-name="${batch.head.tran_head_name}" data-groupe="${batch.tran_groupe_id}" data-quantity="${batch.quantity}" data-mrp="${batch.mrp}" data-tot="${batch.tot_amount}" data-batch="${batch.batch_id}">
                        <td>${batch.head.tran_head_name}</td>
                        <td style="text-align: center">${batch.quantity}</td>
                        <td style="text-align: right">${batch.mrp}</td>
                        <td style="text-align: right">${batch.tot_amount}</td>
                        <td style="text-align: right">${batch.batch_id}</td>
                    </tr>`;
                });
    ...
    function validateAddFormData() {
        ...
        console.log("quantity", quantity > pq)
    ...
    DeleteAjax('Client Return Data Deleted', 'Deleted!');

    PaginationAjax({ type: 6, method: 'Client Return' });

    SearchAjax({ type: 6, method: 'Client Return' });
    
    SearchByDateAjax({ type: 6, method: 'Client Return' });

    SearchPaginationAjax({ type: 6, method: 'Client Return' });
