$(document).ready(function () {
    $(document).on('click', '#permissionCopy', function(){
        $('#copyPermission').show();
        $('#from').val('');
    ...
    $(document).on('click','#AssignCopy', function(e){
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.status == "success") {
                    ...
                    toastr.success('User Permission Copy Successfully', 'Copy!');
                }
            },
            error: function (err) {
                console.log(err)
    ...
    $(document).on('keydown', '#from', function (e) {
    ...
    $(document).on('keydown', '#from-list ul li', function (e) {
        let list = $('#from-list ul li');
    ...
    $(document).on('click', '#from-list li', function () {
        let value = $(this).text();
        let id = $(this).data('id');
        $('#from').val(value);
    ...
    $(document).on('focus', '#from', function (e) {
        let from = $(this).val();
        let id = $(this).attr('data-id');
        if(id == undefined){
    ...
    $(document).on('focusout', '#from', function (e) {
        let id = $(this).attr('data-id');
        if(id == undefined){
    ...
    function getFromUserByName(from, targetElement1) {
        $.ajax({
            url: "/admin/auth/permission/get/from",
            method: 'GET',
            data: { from },
            success: function (res) {
    ...
    $(document).on('keydown', '#to', function (e) {
    ...
    $(document).on('keydown', '#to-list ul li', function (e) {
        let list = $('#to-list ul li');
    ...
    $(document).on('click', '#to-list li', function () {
        let value = $(this).text();
        let id = $(this).data('id');
        $('#to').val(value);
        $('#to').attr('data-id', id);
    ...
    // To User Focus Event
    $(document).on('focus', '#to', function (e) {
        let to = $(this).val();
        let id = $(this).attr('data-id');
        if(id == undefined){
    ...
    $(document).on('focusout', '#to', function (e) {
        let id = $(this).attr('data-id');
        if(id == undefined){
            $(document).on('click', function (e){
                if($(e.target).attr('tabindex') == undefined){
    ...
    function getToUserByName(to, targetElement1) {
        $.ajax({
            url: "/admin/auth/permission/get/to",
            method: 'GET',
            data: { to },
            success: function (res) {
                $(targetElement1).html(res);
