...
<table class="show-table">
    ...
    <tbody>
        ...
        @foreach($data as $key => $item)
            @php
                $itemTotalQuantity = $item->quantity + $item->quantity_issue + $item->quantity_return;
                $itemTotalCostPrice = $itemTotalQuantity * $item->cp;
                $itemTotalAmount = $item->tot_amount - $item->discount;

                $totalQuantity += $itemTotalQuantity;
                $totalCostPrice += $itemTotalCostPrice;
                $totalDiscount += $item->discount;
                $totalTotalAmount += $itemTotalAmount;
            ...
            <tr>
                ...
                <td style="text-align: right">{{ number_format($itemTotalQuantity, 0, '.', ',') }}</td>
                <td style="text-align: right">{{ number_format($item->cp, 0, '.', ',') }}</td>
                <td style="text-align: right">{{ number_format($itemTotalCostPrice, 0, '.', ',') }}</td>
                <td style="text-align: right">{{ number_format($item->discount, 0, '.', ',') }}</td>
                <td style="text-align: right">{{ number_format($itemTotalAmount, 0, '.', ',') }}</td>
