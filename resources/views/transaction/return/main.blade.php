...
@section('main-content')
    {{-- <div class="add-search">
        <div class="rows">
            <div class="c-3">
                 @if(auth()->user()->hasPermission(240))
    ...
    <div class="add-search">
        <div class="rows" style="align-items:center;">
            <div class="c-3">
                @if(Request::segment(1) == 'inventory' && Request::segment(4) == 'client'   && auth()->user()->hasPermission(240))
                            <button class="open-modal" data-modal-id="addModal" id="add"><i class="fa-solid fa-plus"></i> Add {{ $name }} </button>
                @endif

                @if(Request::segment(1) == 'inventory' && Request::segment(4) == 'supplier'   && auth()->user()->hasPermission(244))
                            <button class="open-modal" data-modal-id="addModal" id="add"><i class="fa-solid fa-plus"></i> Add {{ $name }} </button>
                @endif

                @if(Request::segment(1) == 'pharmacy' && Request::segment(4) == 'client'   && auth()->user()->hasPermission(158))
                            <button class="open-modal" data-modal-id="addModal" id="add"><i class="fa-solid fa-plus"></i> Add {{ $name }} </button>
                @endif

                @if(Request::segment(1) == 'pharmacy' && Request::segment(4) == 'supplier'   && auth()->user()->hasPermission(162))
                            <button class="open-modal" data-modal-id="addModal" id="add"><i class="fa-solid fa-plus"></i> Add {{ $name }} </button>
                @endif
            </div>
            <div class="c-3">
    ...
    @include('transaction.return.add')

    @include('transaction.return.edit')
