...
$(document).ready(function () {
    ...
    ReloadData('admin/auth/permission/rolepermissions', ShowRolePermissions);


    //Edit Ajax
    EditAjax('admin/auth/permission/rolepermissions', EditFormInputValue);


    // Update Ajax
    UpdateAjax('admin/auth/permission/rolepermissions', ShowRolePermissions, {}, function() {
        $('#select-all').prop('checked', false);
    });


    // Search Ajax
    SearchAjax('admin/auth/permission/rolepermissions', ShowRolePermissions, {  });


    // Additional Edit Functionality
    function EditFormInputValue(res){
        $('#roleid').text(`Role: ${res.role.name}`);
        $('#role').val(res.role.id);
                ...
                groupedPermissions[groupKey] += `
                        <div class="c-3">
                            <label for="permissions-${item.id}" style="font-weight: normal;">
                                <input type="checkbox" id="permissions-${item.id}" class="permission permission-${item.permission_mainhead}" name="permissions[]" value="${item.id}" ${res.rolepermission.includes(item.id) ? 'checked' : ''} />
                                ${item.name}
                            </label>
                        </div>
                    `;
