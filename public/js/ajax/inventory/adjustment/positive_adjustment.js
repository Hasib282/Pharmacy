function ShowInventoryPositiveAdjustments(res) {
    tableInstance = new GenerateTable({
        ...
        actions: (row) => {
            ...
            if (userPermissions.includes(249) || role == 1) {
                buttons += `
                    <button data-modal-id="editModal" id="edit" data-id="${row.id}"><i class="fas fa-edit"></i></button>
                `;
            }

            if (userPermissions.includes(250) || role == 1) {
                buttons += `
...
$(document).ready(function () {
    ...
    // Insert Ajax
    InsertAjax('inventory/adjustment/positive', 
        {
    ...
    // Delete status  Ajax
    DeleteStatusAjax('inventory/adjustment/positive');


    // Search By Date Ajax
    SearchByDateAjax('inventory/adjustment/positive/search', ShowInventoryPositiveAdjustments, { type: 5, method: 'Positive' });
