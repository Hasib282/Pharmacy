function ShowClients(res) {
    tableInstance = new GenerateTable({
        ...
        actions: (row) => {
            ...
            if (userPermissions.includes(142) || role == 1) {
                buttons += `
                    <button data-modal-id="editModal" id="edit" data-id="${row.id}"><i class="fas fa-edit"></i></button>
                `;
            }

            if (userPermissions.includes(143) || role == 1) {
                buttons += `
...
$(document).ready(function () {
    ...
    renderTableHead([
        ...
        { label: 'Client Type', type:"select", key: 'tran_user_type', method:"fetch", link:"admin/tranwith/get", data:{type:6,user:4}, name:"tran_with_name" },
    ...
    GetTransactionWith(6, null, 4, 'Ok');
    ...
    // Delete status Ajax
    DeleteStatusAjax('pharmacy/users/clients');
