function ShowDetailsReports(res) {
    ...
    tableInstance = new GenerateTable({
        ...
        tbody: [
            ...
            {key:'balance',expration:'receive - payment', type: 'calculate', footerType:'sum'},
        ],
        balance: res.opening,
        rowsPerPage: res.data.length,
    });

    UpdateUrl('/api/report/consolidated/details/print', {type: $("#typeOption").val(),startDate: $('#startDate').val(), endDate: $('#endDate').val() });
}
...
$(document).ready(function () {
    ...
    ReloadData('report/consolidated/details', ShowDetailsReports);


    // Search By Date
    SearchByDateAjax('report/consolidated/details/search', ShowDetailsReports, {type: $("#typeOption").val()});


    // Search by Type
    SearchBySelect('report/consolidated/details/search', ShowDetailsReports, "#typeOption", {type: $("#typeOption").val()});
