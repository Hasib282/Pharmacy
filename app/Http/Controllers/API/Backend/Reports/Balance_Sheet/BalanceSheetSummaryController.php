...
class BalanceSheetSummaryController extends Controller
{
    ...
    public function Print(Request $req){
        ...
        $report_name = 'Balance Sheet Summary';
        $start_date = $req->startDate ? $req->startDate : date('d/m/Y');
        $end_date = $req->endDate ? $req->endDate : date('d/m/Y');
        $pdf = Pdf::loadView('reports.balance_sheet.summary.print', compact('report_name', 'start_date', 'end_date', 'data', 'opening'))->setPaper('a4', 'portrait');
        return $pdf->stream();
    } // End Method
}
