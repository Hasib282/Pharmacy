...
class UserPermissionController extends Controller
{
    ...
    public function ShowAll(Request $req){
        $roles = Role::on('mysql_second')->whereNotIn('id', ['1'])->get();
        $userpermission = Login_User::on('mysql_second')->whereNotIn('user_role', ['1'])->with('permissions')->orderBy('user_name')->paginate(15);
    ...
    public function Edit(Request $req){
        $user = Login_User::on('mysql_second')->whereNotIn('user_role', ['1'])->where('user_id',$req->id)->first();
        $userpermission = Permission_User::on('mysql_second')->where('user_id', $req->id)->pluck('permission_id')->toArray();
    ...
    public function Update(Request $req){
        ...
        $user = Login_User::on('mysql_second')->where('user_id',$req->user)->first();
    ...
    public function Search(Request $req){
        $userpermission = Login_User::on('mysql_second')->whereNotIn('user_role', ['1'])
    ...
    public function UserFrom(Request $req){
        $froms = Login_User::on('mysql_second')->select('user_name','user_id')
    ...
    public function UserTo(Request $req){
        $tos = Login_User::on('mysql_second')->select('user_name','user_id')
    ...
    public function Copy(Request $req){
        ...
        $from = Login_User::on('mysql_second')->where('user_id', $req->from)->first();
        $to = Login_User::on('mysql_second')->where('user_id', $req->to)->first();
