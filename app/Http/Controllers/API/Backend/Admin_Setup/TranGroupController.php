...
class TranGroupController extends Controller
{
    ...
    public function ShowAll(Request $req){
        $groupes = Transaction_Groupe::with('Type')->orderBy('added_at')->paginate(15);
    ...
    public function Edit(Request $req){
        $groupes = Transaction_Groupe::with('Type')->findOrFail($req->id);
        $types = Transaction_Main_head::orderBy('added_at')->get();
        return response()->json([
            'status'=> true,            
            'groupes'=>$groupes,
    ...
    public function Search(Request $req){
        $groupes = Transaction_Groupe::with('Type')
