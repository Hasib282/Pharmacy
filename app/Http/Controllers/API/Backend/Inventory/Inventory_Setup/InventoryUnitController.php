...
class InventoryUnitController extends Controller
{
    ...
    public function ShowAll(Request $req){
        $unit = Item_Unit::on('mysql_second')->where('type_id', '5')->orderBy('added_at','asc')->paginate(15);
    ...
    public function Search(Request $req){
        $unit = Item_Unit::on('mysql_second')->where('unit_name', 'like', '%'.$req->search.'%')
        ->where('type_id', 5)
    ...
    public function Get(Request $req){
        $units = Item_Unit::on('mysql_second')->where('type_id', '5')
