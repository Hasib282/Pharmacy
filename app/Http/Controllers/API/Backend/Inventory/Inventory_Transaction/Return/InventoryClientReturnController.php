...
class InventoryClientReturnController extends Controller
{
    ...
    public function ShowAll(Request $req){
        $return = Transaction_Main::on('mysql_second')->with('User')->where('tran_method','Client Return')->where('tran_type','5')->whereRaw("DATE(tran_date) = ?", [date('Y-m-d')])->orderBy('tran_date','asc')->paginate(15);
