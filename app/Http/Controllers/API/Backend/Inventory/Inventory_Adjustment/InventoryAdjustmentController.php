...
class InventoryAdjustmentController extends Controller
{
    ...
    public function ShowAllPositive(Request $req){
        $adjust = Transaction_Detail::with('Store','Head')->where('tran_method','Positive')->where('tran_type','5')->whereRaw("DATE(tran_date) = ?", [date('Y-m-d')])->orderBy('tran_date','asc')->paginate(15);
    ...
    public function ShowAllNegative(Request $req){
        $adjust = Transaction_Detail::with('Store','Head')->where('tran_method','Negative')->where('tran_type','5')->whereRaw("DATE(tran_date) = ?", [date('Y-m-d')])->orderBy('tran_date','asc')->paginate(15);
        $groupes = Transaction_Groupe::where('tran_groupe_type', '5')->orderBy('added_at','asc')->get();
        return response()->json([
            'status'=> true,
            'data' => $adjust,
    ...
    public function Search(Request $req){
        if($req->searchOption == 1){
            $adjust = Transaction_Detail::with('Store','Head')
