...
class PharmacyClientReturnController extends Controller
{
    ...
    public function ShowAll(Request $req){
        $return = Transaction_Main::with('User')->where('tran_method','Client Return')->where('tran_type','6')->whereRaw("DATE(tran_date) = ?", [date('Y-m-d')])->orderBy('tran_date','asc')->paginate(15);
    ...
    public function Insert(Request $req){
        ...
        DB::transaction(function () use ($req, $id) {
            ...
            foreach($products as $product){
                ...
                $batch = Transaction_Detail::where('tran_id', $req->batch)->where('tran_head_id', $product['product'])->where('batch_id', $product['batch'])->first();
                ...
                $batch = Transaction_Detail::where('tran_id', $product['batch'])->where('tran_head_id', $product['product'])->first();
                $rem_quantity = $batch->quantity + $product['quantity'];
