...
class PayrollMiddlewireController extends Controller
{
    ...
    public function ShowAll(Request $req){
        ...
        $payroll = Pay_Roll_Middlewire::on('mysql')->with('Employee','Head')
            ->orderBy('emp_id','asc')
    ...
    public function Insert(Request $req){
        ...
        $middlewire = Pay_Roll_Middlewire::on('mysql')->where('emp_id', $req->user)
        ...
        $setup = Pay_Roll_Setup::on('mysql')->where('emp_id', $req->user)
                    ->where('head_id', $req->head)
                    ->count();
        ...
        if($middlewire > 0 || $setup > 0){
            ...
            Pay_Roll_Middlewire::on('mysql')->insert([
                "emp_id" => $req->user,
                "head_id" => $req->head,
                "amount" => $req->amount,
                "date" => $date,
    ...
    public function Edit(Request $req){
        $payroll = Pay_Roll_Middlewire::on('mysql')->with('Employee', 'Head')->where('id',$req->id)->findOrFail($req->id);
        $tranwith = Transaction_With::on('mysql')->where('user_role', 3)->get();
        $heads = Transaction_Head::on('mysql_second')->where('groupe_id','1')->get();
        return response()->json([
            'status'=> true,
    ...
    public function Update(Request $req){
        ...
        if($middlewire > 0 || $setup > 0){
            ...
            $update = Pay_Roll_Middlewire::on('mysql')->findOrFail($req->id)->update([
                "emp_id" => $req->user,
                "head_id" => $req->head,
                "amount" => $req->amount,
                "date" => $req->date,
    ...
    public function Delete(Request $req){
        Pay_Roll_Middlewire::on('mysql')->findOrFail($req->id)->delete();
        return response()->json([
            'status'=> true,
